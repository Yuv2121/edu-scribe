...
# Check whether GoogleSearch tool is available in the agent module.
# agent.py imports GoogleSearch (or sets it to None) so we can inspect it here.
import importlib
agent_mod = importlib.import_module('agent')
GoogleSearch = getattr(agent_mod, 'GoogleSearch', None)

# Honor strict mode: when set, fail early if GoogleSearch is required but missing.
strict_mode = os.getenv('EDUSCRIBE_STRICT_GOOGLESEARCH', '').strip() in ('1', 'true', 'True')
if GoogleSearch is None:
    print('\n⚠️  GoogleSearch tool not detected in the installed ADK.')
    print('Hybrid retrieval (Pedagogy Vectorstore + Google search) will run in limited mode.')
    print('To enable GoogleSearch:')
    print('  - Install/upgrade the ADK or the tools package that provides GoogleSearch,')
    print('    or add a compatible tool implementation into `tools.py`.')
    print('  - Optionally set EDUSCRIBE_STRICT_GOOGLESEARCH=1 to enforce availability.')
    if strict_mode:
        print('\nEDUSCRIBE_STRICT_GOOGLESEARCH is set — aborting because GoogleSearch is required.')
        sys.exit(2)
